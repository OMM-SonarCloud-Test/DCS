...
public class KmcSdlsEngine {
    ...
    private void configureKmcSdlsEngineFromProperties(Properties props) throws Exception {
        ...
        if (cryptographyTypeEnum == cryptographyType.KMC_CRYPTO_SERVICE) {
            ...
            if (kmcCryptoHostname == null) {
                kmcCryptoHostname = "localhost";
            }
            if (kmcCryptoApp == null) {
                kmcCryptoApp = "crypto-service";
            }
            if (mtlsClientCertType == null) {
                mtlsClientCertType = "PEM";
            }
    ...
    public int configureCryptoLib(sadbType sadbTypeEnum, cryptographyType cryptographyTypeEnum, boolean cryptoCreateFecf, boolean processSdlsPdus, boolean hasPusHdr, boolean ignoreSaState, boolean ignoreAntiReplay, boolean uniqueSaPerMapid, boolean cryptoCheckFecf, int vcidBitmask, boolean cryptoIncrementNontransmittedIv) throws Exception {
        short sadb_type, cryptography_type, crypto_create_fecf, process_sdls_pdus, has_pus_hdr, ignore_sa_state, ignore_anti_replay, unique_sa_per_mapid, crypto_check_fecf, vcid_bitmask, crypto_increment_nontransmitted_iv;
        ...
        if (cryptoCreateFecf) crypto_create_fecf = (short) CreateFecfBool.CRYPTO_TC_CREATE_FECF_TRUE.swigValue();
        else crypto_create_fecf = (short) CreateFecfBool.CRYPTO_TC_CREATE_FECF_FALSE.swigValue();

        if (processSdlsPdus) process_sdls_pdus = (short) TcProcessSdlsPdus.TC_PROCESS_SDLS_PDUS_TRUE.swigValue();
        else process_sdls_pdus = (short) TcProcessSdlsPdus.TC_PROCESS_SDLS_PDUS_FALSE.swigValue();

        if (hasPusHdr) has_pus_hdr = (short) TcPusHdrPresent.TC_HAS_PUS_HDR.swigValue();
        else has_pus_hdr = (short) TcPusHdrPresent.TC_NO_PUS_HDR.swigValue();

        if (ignoreSaState) ignore_sa_state = (short) TcIgnoreSaState.TC_IGNORE_SA_STATE_TRUE.swigValue();
        else ignore_sa_state = (short) TcIgnoreSaState.TC_IGNORE_SA_STATE_FALSE.swigValue();

        if (ignoreAntiReplay) ignore_anti_replay = (short) TcIgnoreAntiReplay.TC_IGNORE_ANTI_REPLAY_TRUE.swigValue();
        else ignore_anti_replay = (short) TcIgnoreAntiReplay.TC_IGNORE_ANTI_REPLAY_FALSE.swigValue();

        if (uniqueSaPerMapid) unique_sa_per_mapid = (short) TcUniqueSaPerMapId.TC_UNIQUE_SA_PER_MAP_ID_TRUE.swigValue();
        else unique_sa_per_mapid = (short) TcUniqueSaPerMapId.TC_UNIQUE_SA_PER_MAP_ID_FALSE.swigValue();

        if (cryptoCheckFecf) crypto_check_fecf = (short) CheckFecfBool.TC_CHECK_FECF_TRUE.swigValue();
        else crypto_check_fecf = (short) CheckFecfBool.TC_CHECK_FECF_FALSE.swigValue();
        ...
        int status = this.kmcCInterface.sdls_config_cryptolib(sadb_type, cryptography_type, crypto_create_fecf, process_sdls_pdus, has_pus_hdr, ignore_sa_state, ignore_anti_replay, unique_sa_per_mapid, crypto_check_fecf, vcid_bitmask, crypto_increment_nontransmitted_iv);
    ...
    public int addGvcidManagedParameter(int transferFrameVersionNumber, int spacecraftId, int virtualChannelID, boolean hasFecf, boolean hasSegmentationHdr, int maxTcFrameSize) throws Exception {
        ...
        if (hasFecf) has_fecf = (short) FecfPresent.TC_HAS_FECF.swigValue();
        else has_fecf = (short) FecfPresent.TC_NO_FECF.swigValue();
        if (hasSegmentationHdr) has_segmentation_hdr = (short) TcSegmentHdrsPresent.TC_HAS_SEGMENT_HDRS.swigValue();
        else has_segmentation_hdr = (short) TcSegmentHdrsPresent.TC_NO_SEGMENT_HDRS.swigValue();
    ...
    public int configureCryptoService(String protocol, String kmcCryptoHostname, int kmcCryptoPort, String kmcCryptoApp, String kmcTlsCaBundle, String kmcTlsCaPath, boolean kmcIgnoreSslHostnameValidation, String mtlsClientCertPath, String mtlsClientCertType, String mtlsClientKeyPath, String mtlsClientKeyPass, String mtlsIssuerCert) throws Exception {
        short kmc_crypto_port, kmc_ignore_ssl_hostname_validation;
        kmc_crypto_port = (short) kmcCryptoPort;
        if (kmcIgnoreSslHostnameValidation) kmc_ignore_ssl_hostname_validation = (short) 1;
        else kmc_ignore_ssl_hostname_validation = (short) 0;

        int status = this.kmcCInterface.sdls_config_kmc_crypto_service(protocol, kmcCryptoHostname, kmc_crypto_port, kmcCryptoApp, kmcTlsCaBundle, kmcTlsCaPath, kmc_ignore_ssl_hostname_validation, mtlsClientCertPath, mtlsClientCertType, mtlsClientKeyPath, mtlsClientKeyPass, mtlsIssuerCert);
        this.engineStatus = status;
    ...
    public int configureCam(boolean camEnabled, String cookieFilePath, String keytabFilePath, loginMethod loginMethodEnum, String accessManagerUri, String username, String camHome) throws Exception {
        short cam_enabled, login_method;

        // Prepare the java parameters for the Configure CAM JNI call.

        if (camEnabled) cam_enabled = (short) CamEnabledBool.CAM_ENABLED_TRUE.swigValue();
        else cam_enabled = (short) CamEnabledBool.CAM_ENABLED_FALSE.swigValue();
        ...
        int status = this.kmcCInterface.sdls_config_cam(cam_enabled, cookieFilePath, keytabFilePath, login_method, accessManagerUri, username, camHome);
        this.engineStatus = status;
